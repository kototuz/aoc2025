...
main() {
    ...
    auto i; i = 0; while (i < numbers_count) {
        printf("#%3zu: %16zu %16zu\n", i, numbers[i], numbers[i+1]);
        printf("++++++++++++++++++++++++++++++++++++++++++++++++++++\n");
        if (numbers[i] != 0) {
            auto j; j = 0; while (j < numbers_count) {
                printf("#%3zu: %16zu %16zu ", j, numbers[j], numbers[j+1]);
                if (j == i) {
                    printf("CURRENT\n");
                } else if (numbers[j] == 0) {
                    printf("REMOVED\n");
                } else if (numbers[i] <= numbers[j] & numbers[j+1] <= numbers[i+1]) {
                    printf("REMOVING\n");
                    numbers[j] = 0;
                } else if (numbers[i] <= numbers[j+1] & numbers[j+1] <= numbers[i+1] & numbers[j] < numbers[i]) {
                    printf("RIGHT=%zu\n", numbers[i]-1);
                    numbers[j+1] = numbers[i]-1;
                } else if (numbers[i] <= numbers[j] & numbers[j] <= numbers[i+1] & numbers[j+1] > numbers[i+1]) {
                    printf("LEFT=%zu\n", numbers[i+1]+1);
                    numbers[j] = numbers[i+1]+1;
                } else {
                    printf("\n");
                }
